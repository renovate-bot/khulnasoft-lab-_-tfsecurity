...
func Test_Flag_Version(t *testing.T) {
	...
	assert.Equal(t, "", err)
	assert.Equal(t, "You are running a locally built version of tfsecurity.\n", out)
	assert.Equal(t, 0, exit)
}
...
func Test_Flag_Out(t *testing.T) {
	tmp, err := os.MkdirTemp(os.TempDir(), "tfsecurity")
	require.NoError(t, err)
	defer func() { _ = os.RemoveAll(tmp) }()
	file := filepath.Join(tmp, "tfsecurity_output.json")
