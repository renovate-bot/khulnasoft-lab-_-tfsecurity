...
func prerun(cmd *cobra.Command, args []string) error {
	...
	if showVersion {
		if version.Version == "" {
			_, _ = fmt.Fprintln(cmd.OutOrStdout(), "You are running a locally built version of tfsecurity.")
		} else {
	...
	if migrateIgnores {
		...
		if err != nil {
			return fmt.Errorf("directory was not provided, and tfsecurity encountered an error trying to determine the current working directory: %w", err)
		}
