...
func applyConfigFiles(options []options.ScannerOption, dir string) ([]options.ScannerOption, error) {
	...
	if configFile == "" {
		configDir := filepath.Join(dir, ".tfsecurity")
	...
	if configFile != "" {
		if conf, err := config.LoadConfig(configFile); err == nil {
			...
			if !minVersionSatisfied(conf) {
				return nil, fmt.Errorf("minimum tfsecurity version requirement not satisfied")
			}
...
func configureCustomChecks(options []options.ScannerOption, dir string) ([]options.ScannerOption, error) {
	...
	if customCheckDir == "" {
		customCheckDir = filepath.Join(dir, ".tfsecurity")
	}
...
func remoteCustomCheckDownloaded() bool {
	customTempDir, err := os.MkdirTemp(os.TempDir(), fmt.Sprintf("tfsecurity_custom_check_%s", uuid.NewString()))
	if err != nil {
		return false
	}
