...
| value              | In cases where a value is required, the value to look for, text matching `TFSECURITY_VAR_{VAR_NAME}` will be replaced with the variable value                              |
...
| assignVariable     | The name of the "variable" to store the value of the `name` attribute in, has to be in uppercase and start with `TFSECURITY_VAR_`                                          |
