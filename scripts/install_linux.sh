...
checksum_file="tfsecurity_checksums.txt"
download_path=$(mktemp -d -t tfsecurity.XXXXXXXXXX)
...
if [[ $remote_filename == *"windows"* ]]; then
  ...
  retVal=$?
  if [ $retVal -ne 0 ]; then
    echo "Failed to install tfsecurity"
    exit $retVal
  else
...
echo -e "\n\n===================================================="
echo "Current tfsecurity version: $(${dest}${local_filename} -v)"
